...
function assignStyle(el, ...args) {
    ...
    function _assignStyle(el, style) {
        if (isString(style)) {
            if (isCssWord(style)) {
...
function raf(fn) {
     (window.requestAnimationFrame       ||
         ...
         (function(x) {setInterval(x, 1000/60); }))(fn)
}
...
function cssLoader(s) {
    let delay = getStackTrace().length == 3 ? 250 : 0
    //return sleep(delay).then((x) => _cssLoader(s))
    return new Promise(resolve => {
        setTimeout(() => {
            const value = _cssLoader(s)
...
function _cssLoader(s) {
    ...
    const value = /^\s*<style/.test(s) ?
        search(/<style>([^]+?)<\/style>/, s) : s

    if (!/^@|}$/m.test(value)) return 
    console.log('cssLoader', value)
    return createElement({tag: 'style', html: value})
}
            ...
            (e)  => {
              ...
              if (key in state) {
                  ...
                  if (isFunction(fn)) {
                      ...
                      console.log('huhhh', key, fn, state)
                  }
              }
            }
...
function htmlSkeleton(body) {
    if (!body) body = document.body
    const s = body.innerHTML
    const key = 'html'
    return loadPrettier(key).then((x) => pretty(s, key))
}
...
function vueLoader(c, autoload, delay = 50) {
    window.addEventListener('load', () => {
        fixVH()
...
function _skel() {
    const ignore = ['HTMLScriptElement', 'HTMLStyleElement', 'HTMLLinkElement']
...
const skel = timeoutf(logf(_skel), 250)
...
function replaceArrayItem(items, a, b) {
    var index = isFunction(a) ? items.findIndex(a) : items.indexOf(a)
