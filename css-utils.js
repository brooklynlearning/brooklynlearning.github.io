...
function aggregateCSS(s, mode = String) {
    /* need a way to get rid of the empty padding 0or margin 0 */
    /* how will it change */
    /* it will go through phases, and take something each time */
    /* children of poverty experience poverty every day */
    const regex = /^(.*?)\s*{([^]+?)}/gm
    ...
    findall(regex, s).forEach(([a, b]) => {
        //console.log([a, b])
        a = a.trim()
        if (test(/foo|boo|mockup/, a)) {
            console.log('skipping foo and boo and mockups')
            console.log('allowed for now')
            //return
        }
        const pairs = fn(b)
...
function cssValueParser(a, b) {
    ...
    switch (a) {
        case 't': return [['bottom', 'unset'], ['top', b]]
        case 'l': return [['right', 'unset'], ['left', b]]
        case 'b': return [['top', 'unset'], ['bottom', b]]
        case 'r': return [['left', 'unset'], ['right', b]]
        case 'tx':
        case 'ty':
        case 'r':
...
function cssTextShadow(b) {
    let color = '#000'
    return b.split(/, */).map((item, i) => {
        let [sign, d1, d2] = search(/(-)*(\d)(\d*)/, item)
        if (!sign) sign = ''
        return `${sign}${d1}px ${sign}${d2 || d1}px ${color}`
    }).join(', ')
}
...
function cssToString(x) {
    let iterable = getConstructorName(x) == 'Storage' ? 
        storage.entries :
        stop()
...
function cssCleanupFinalString(s) {
    ...
    let filterMargin
    let filterPadding
    if (lastMargin && (lastMargin[0] == 'margin' && lastMargin[1] == 0)) {
        filterMargin = true
    }
    let padding = match(/((?:padding)(?:-\w+)?): (\w+)/g, s)
    let lastPadding = getLast(padding)
    if (lastPadding && (lastPadding[0] == 'padding' && lastPadding[1] == 0)) {
        filterPadding = true
    }
...
function cssHasStringError(s) {
    return s && /outline|unset|none|null|undefined|: (?:[=]|[a-zA-Z]{1,2};)/.test(s)
}
...
function gradientColor(b) {
    return match(/\w\w/g, b).map(partial(cssColor, false))
        .join(', ')
}


function cssGradient(b) {
    const value = `linear-gradient(to right, ${gradientColor(b)})`
    return [
        ['-webkit-background-clip', 'text'],
        ['-webkit-text-fill-color', 'transparent'],
...
//console.log(cssEvaluator('mb60'))

console.log(cssQuery('medium', 'hi'))
